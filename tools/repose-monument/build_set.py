...
col=build_column()
cap=build_fallen_capital()
drum=build_drum()
alt=build_altar()
# lay fragments flat where useful is done in Three; keep at origin here

# marble on all
for ob in (col,cap,drum,alt):
    B.marble(ob, base=(0.86,0.85,0.82))

if "--render" in sys.argv:
    B.set_world_hdri("/tmp/statues/quarry_01_1k.hdr",0.6)
    # line the fragments up for a lookdev sheet
    cap.location=(-2.2,0,0.3); cap.rotation_euler=(0,0,0.4)
    drum.location=(2.2,0,0.42); drum.rotation_euler=(1.4,0,0.2)
    alt.location=(4.2,0,0)
    t=bpy.data.objects.new("t",None); bpy.context.scene.collection.objects.link(t); t.location=(1.0,0,1.6)
    cd=bpy.data.cameras.new("c"); cd.lens=40; cd.clip_end=1000
    c=bpy.data.objects.new("c",cd); bpy.context.scene.collection.objects.link(c); c.location=(1.0,-11,2.2)
    cn=c.constraints.new('TRACK_TO'); cn.target=t; cn.track_axis='TRACK_NEGATIVE_Z'; cn.up_axis='UP_Y'
    bpy.context.scene.camera=c
    B.render("/tmp/set.png",1400,900,40)
    print("SET RENDER DONE")
else:
    bpy.ops.object.select_all(action='DESELECT')
    for ob in (col,cap,drum,alt): ob.select_set(True)
    for ob in (col,cap,drum,alt): ob.data.materials.clear()
    bpy.ops.export_scene.gltf(filepath="/tmp/set_raw.glb", export_format='GLB', use_selection=True,
        export_apply=True, export_yup=True, export_normals=True)
    print("SET EXPORT", "colV",len(col.data.vertices))
